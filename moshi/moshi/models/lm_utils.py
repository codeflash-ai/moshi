...
def _undelay_sequence(
    ...
    # Early exit if all delays are zero
    if all(delay == 0 for delay in delays):
        return tensor, mask

    # Prepare for per-key shifting
    outs = torch.empty_like(tensor)
    for k, delay in enumerate(delays):
        assert delay >= 0
        if delay == 0:
            outs[:, k] = tensor[:, k]
            continue
        # Use torch.roll in batch for all B, not per slice to reduce overhead
        outs[:, k] = torch.roll(tensor[:, k], shifts=-delay, dims=1)
        outs[:, k, -delay:] = fill_value
        mask[:, k, -delay:] = 0
    return outs, mask
...
def _init_layer(m: nn.Module, zero_bias_init: bool = True):
