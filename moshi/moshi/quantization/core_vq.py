...
class VectorQuantization(nn.Module):
    ...
    def _rearrange_output(self, quantized):
        # Optimized the rearrangement using torch.permute for better performance
        # Assumes quantized is a torch.Tensor of shape (b, n, d)
        # Equivalent to einops rearrange(quantized, "b n d -> b d n")
        if isinstance(quantized, torch.Tensor) and quantized.ndim == 3:
