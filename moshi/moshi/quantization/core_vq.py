...
class EuclideanCodebook(nn.Module):
    ...
    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        # Use self._embedding instead of self.embedding if available, avoids unnecessary recomputation on property access
        # But preserve semantics: .embedding is preferred, as set by other logic
    ...
    @property
    def embedding(self) -> torch.Tensor:
        # Small speed: prefer an in-memory cached version if _embedding is available and not None, else use embedding_sum / cluster_usage
        if hasattr(self, "_embedding") and self._embedding is not None:
            return self._embedding
        # fallback: (the actual computation is not in the provided code, but in practical code this would recalc on-the-fly)
        return self.embedding_sum / (
            self.cluster_usage.unsqueeze(-1).clamp(min=self.epsilon)
        )
...
class VectorQuantization(nn.Module):
    ...
    def _rearrange_output(self, quantized):
        # Use optimized rearrange if possible
        return self._fast_rearrange_output(quantized)
    ...
    @staticmethod
    def _fast_rearrange_output(quantized: torch.Tensor) -> torch.Tensor:
        # Optimized replacement for einops.rearrange(quantized, "b n d -> b d n")
        # Only for 3D [b, n, d] -> [b, d, n], else fallback to einops
        # Rearranging via permute is much faster than einops for this common pattern
        if quantized.ndim == 3:
            return quantized.permute(0, 2, 1)
        else:
            # Import here to avoid a runtime dependency if not needed
